...
<head>
    <meta charset="utf-8">
    <title>{{ $invoice->invoice_number }} | Loudon Mechanical Services</title>
    ...
    <style>
        ...
        .top { display:flex; justify-content:space-between; gap:24px; border-bottom:4px solid #0e8fe8; padding-bottom:22px; margin-bottom:24px; }
...
<body>
    ...
    <div class="page">
        <div class="top">
            ...
            <div class="meta">
                ...
                @if ($invoice->service_date)
                    <p>Service Date: {{ $invoice->service_date->format('m/d/Y') }}</p>
                @endif
                @if ($invoice->due_date)
        ...
        <div class="grid">
            ...
            <div class="box">
                ...
                @if ($invoice->serviceLocation)
                    <p>{{ $invoice->serviceLocation->location_name }}</p>
                    <p>{{ $invoice->serviceLocation->address }}</p>
                    <p>{{ $invoice->serviceLocation->city }} {{ $invoice->serviceLocation->state }} {{ $invoice->serviceLocation->postal_code }}</p>
                @else
        ...
        <div class="totals">
            ...
            <div><span>Tax</span><strong>${{ number_format($invoice->tax, 2) }}</strong></div>
            <div><span>Total</span><strong>${{ number_format($invoice->total, 2) }}</strong></div>
            <div><span>Paid</span><strong class="paid">${{ number_format($invoice->amount_paid, 2) }}</strong></div>
            <div class="total"><span>Balance</span><strong class="balance">${{ number_format($invoice->balance, 2) }}</strong></div>
        </div>

        <p class="muted" style="margin-top:32px">Thank you for choosing Loudon Mechanical Services.</p>
    </div>
</body>
</html>
